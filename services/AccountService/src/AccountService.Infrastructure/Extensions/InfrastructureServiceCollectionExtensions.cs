...
using AccountService.Infrastructure.Authentication.Services;
...
{
    private const string InfraRepositoriesNamespace = "AccountService.Infrastructure.Persistence.Repositories";
    private const string InfraRepositoryInterfacesNamespace = "AccountService.Domain.Interfaces.Repositories";
    ...
    {
        var infrastructureAssembly = Assembly.Load("AccountService.Infrastructure")
            ?? throw new InvalidOperationException("Could not find Infrastructure assembly");

        var domainAssembly = Assembly.Load("AccountService.Domain")
